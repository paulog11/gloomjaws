 ...
 */
export function determineFocus(monster, players, grid, attackRange) {
    ...
    for (const player of alivePlayers) {
        if (hasCondition(player, ConditionType.INVISIBLE))
            continue;
    ...
    candidates.sort((a, b) => {
        if (a.movesNeeded !== b.movesNeeded)
            return a.movesNeeded - b.movesNeeded;
        if (a.player.initiative !== b.player.initiative)
            return a.player.initiative - b.player.initiative;
        const aSpace = grid.getById(a.player.spaceId);
        const bSpace = grid.getById(b.player.spaceId);
        if (!aSpace || !bSpace)
            return 0;
        return (axialDistance(monsterSpace.coord, aSpace.coord) -
 ...
 */
export function resolveMonsterTurn(monster, players, grid, moveValue, attackValue, attackRange, queue, onMove, onAttack) {
 ...
 */
export function drawMonsterAICard(aiDeck, discard) {
    if (aiDeck.length === 0) {
