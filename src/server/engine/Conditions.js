...
export function hasCondition(actor, type) {
    return actor.conditions.some(c => c.type === type);
}
export function applyCondition(actor, type, sourceId) {
...
export function removeCondition(actor, type) {
...
export function removeAllConditions(actor) {
    actor.conditions = [];
}
/**
 ...
 */
export function tickConditions(actor) {
    let poisonDamage = 0;
    let woundDamage = 0;
 ...
 */
export function getAttackModifierFromConditions(actor) {
    let mod = 0;
    if (hasCondition(actor, ConditionType.MUDDLE))
        mod -= 1;
    if (hasCondition(actor, ConditionType.STRENGTHEN))
        mod += 1;
    return mod;
}
/**
 ...
 */
export function isDisarmed(actor) {
