...
import { ModifierCard } from '../../common/types.js'

export interface ExecutionContext {
  source: IActorBase
  targets: IActorBase[]
  grid: HexGrid  // available for positional logic (push/pull resolution)
  elements: ElementState
  getModifierDeck: (actorId: string) => { deck: ModifierCard[]; discard: ModifierCard[] }
  onDamage: (target: IActorBase, amount: number) => void
  onHeal: (target: IActorBase, amount: number) => void
  onDeath: (actor: IActorBase) => void
  onLoot: (actor: IActorBase, spaceId: string) => void
 ...
 */
export function canExecute(behavior: Behavior, ctx: ExecutionContext): boolean {
  const { source } = ctx

  if (behavior.attack && isDisarmed(source)) return false

  if (behavior.consumeElement) {
    const strength = ctx.elements[behavior.consumeElement]
    if (strength === ElementStrength.INERT) return false
  }
 ...
 */
export function execute(behavior: Behavior, ctx: ExecutionContext): void {
  ...
  if (behavior.xp && source.type === ActorType.PLAYER) {
    const player = source as IPlayerBase
    player.xp += behavior.xp
    addLog(source.id, `gained ${behavior.xp} XP`)
  }
}

function getShieldValue(_actor: IActorBase): number {
