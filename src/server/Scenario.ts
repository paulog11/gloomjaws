...
import { IMonsterDef } from '../common/types.js'

export class Scenario {
  def: IScenarioDef
  spaces: ISpace[]
  revealedRooms: number[]
  monsters: Map<string, IMonsterToken> = new Map()
  monsterAIDecks: Map<string, { deck: MonsterAICard[]; discard: MonsterAICard[] }> = new Map()

  constructor(def: IScenarioDef) {
    this.def = def
  ...
  spawnMonsters(
    ...
    for (const spawn of this.def.monsterSpawns) {
      if (!this.revealedRooms.includes(spawn.roomId)) continue
      ...
      if (space) space.occupantId = token.id
  ...
  revealRoom(roomId: number, monsterDefs: Map<string, IMonsterDef>, scenarioLevel: number): void {
    if (this.revealedRooms.includes(roomId)) return
    this.revealedRooms.push(roomId)
    this.spawnMonsters(monsterDefs, scenarioLevel, 0)
  }

  getMonsterStats(monsterId: string, isElite: boolean, scenarioLevel: number, monsterDefs: Map<string, IMonsterDef>) {
  ...
  drawAICardForGroup(monsterId: string): MonsterAICard | null {
  ...
  removeMonster(monsterId: string): void {
    ...
    if (monster.spaceId) {
      ...
      if (space) space.occupantId = undefined
    }
