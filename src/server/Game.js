...
import { Element, ElementStrength, ScenarioResult, ActorType, } from '../common/types.js';
import { Player } from './Player.js';
import { Scenario } from './Scenario.js';
import { HexGrid } from './engine/HexGrid.js';
import { DeferredQueue } from './engine/DeferredQueue.js';
import { execute, canExecute } from './engine/Executor.js';
...
export class Game {
    ...
    scenarioId;
    scenarioLevel;
    scenarioResult = ScenarioResult.IN_PROGRESS;
    players = new Map();
    scenario;
    monsterDefs;
    ...
    grid;
    constructor(scenarioDef, monsterDefs, scenarioLevel = 1) {
        this.gameId = uuidv4();
    ...
    processCardSelection(playerId, topCardId, bottomCardId) {
    ...
    transitionToInitiativeReveal() {
        // Draw AI cards for each monster group
        const monsterGroups = new Set(Array.from(this.scenario.monsters.values()).map(m => m.monsterId));
    ...
    // ─── Phase: Round Actions ─────────────────────────────────────────────────
    processPlayerAction(payload) {
        const player = this.getPlayerByPlayerId(payload.playerId);
        if (!player)
            throw new Error(`Player ${payload.playerId} not found`);
        if (this.activeActorId !== player.id)
            throw new Error('Not this player\'s turn');
        switch (payload.action) {
            case 'SELECT_CARDS':
                this.processCardSelection(payload.playerId, payload.data?.topCardId, payload.data?.bottomCardId);
                break;
            case 'MOVE': {
                const toSpaceId = payload.data?.spaceId;
                if (!toSpaceId)
                    throw new Error('spaceId required for MOVE');
            ...
            case 'ATTACK': {
                ...
                const useTop = payload.data?.useTop;
                if (!targetId || !cardId)
                    throw new Error('targetId and cardId required for ATTACK');
    ...
    moveActor(player, toSpaceId) {
        const fromSpace = this.scenario.spaces.find(s => s.id === player.spaceId);
        const toSpace = this.scenario.spaces.find(s => s.id === toSpaceId);
        if (!toSpace)
            throw new Error(`Space ${toSpaceId} not found`);
        if (!this.grid.isPassable(toSpace.coord))
            throw new Error(`Space ${toSpaceId} is not passable`);
        if (fromSpace)
            fromSpace.occupantId = undefined;
    ...
    playerAttack(player, cardId, useTop, targetId) {
        ...
        const ctx = {
            ...
            getModifierDeck: (actorId) => {
                ...
                if (p)
                    return { deck: p.modifierDeck, discard: p.modifierDiscard };
                return { deck: [], discard: [] };
            },
            onDamage: (t, amount) => this.applyDamage(t, amount),
            onHeal: (t, amount) => this.applyHeal(t, amount),
            onDeath: (a) => this.handleDeath(a),
            onLoot: (a, spaceId) => this.handleLoot(a, spaceId),
    ...
    autoResolveMonsterTurn() {
        while (this.activeActorId) {
            const monster = this.scenario.monsters.get(this.activeActorId);
            if (!monster)
                break; // It's a player's turn
            const stats = this.scenario.getMonsterStats(monster.monsterId, monster.isElite, this.scenarioLevel, this.monsterDefs);
            if (!stats) {
                this.endActorTurn(monster.id);
                continue;
            }
            const players = Array.from(this.players.values());
            resolveMonsterTurn(monster, players, this.grid, stats.move, stats.attack, stats.range, this.deferredQueue, (m, toSpaceId) => {
                const fromSpace = this.scenario.spaces.find(s => s.id === m.spaceId);
                const toSpace = this.scenario.spaces.find(s => s.id === toSpaceId);
                if (fromSpace)
                    fromSpace.occupantId = undefined;
                if (toSpace) {
                    toSpace.occupantId = m.id;
                    m.spaceId = toSpaceId;
                }
                this.grid = new HexGrid(this.scenario.spaces);
    ...
    transitionToEndOfRound() {
        ...
        for (const player of this.players.values()) {
            ...
            if (poisonDamage > 0)
                this.applyDamage(player, poisonDamage);
            if (woundDamage > 0)
                this.applyDamage(player, woundDamage);
        }
        for (const monster of this.scenario.monsters.values()) {
            ...
            if (woundDamage > 0)
                this.applyDamage(monster, woundDamage);
            monster.acted = false;
        }
        // Decay elements
    ...
    handleDeath(actor) {
        if (actor.type === ActorType.MONSTER) {
            this.addLog(actor.id, `${actor.monsterId} #${actor.number} defeated`);
    ...
    findActor(actorId) {
        if (this.players.has(actorId))
            return this.players.get(actorId);
        if (this.scenario.monsters.has(actorId))
            return this.scenario.monsters.get(actorId);
        return null;
    }
